<template>
  <div class="feeds">
    <!-- MOBILE MODAL SCREENS-->
    <!-- <div id="modal1" class="modal hide-on-large-only">
      ...
      <div v-show="showStatus" class="item-text">Status</div>
      <div v-show="showForum" class="item-text">Forum</div>
      <div v-show="showNews" class="item-text">News</div>
      <div class="modal-2">
        <div class="modal-content">
          <v-col cols="12" sm="6" md="3">
            <v-text-field placeholder="Add title or question (Required)"></v-text-field>
            <v-container fluid>
              <v-textarea counter placeholder="Add note to your post(optional)"></v-textarea>
            </v-container>
          </v-col>
        </div>
        ...
        <div class="modal-footer">
          <div class="bottom-modal">
            ...
            <div v-show="showAudio || showVideo" class="forum-category">
              <select class="select-category browser-default" v-model="selected_forum_category">
                <option disabled value>Category</option>
                ...
                <option>Reggae1</option>
                <option>Gospel</option>
              </select>
            </div>
            <div @click="hide" class="publish-button modal-close">
              <div>Publish</div>
            </div>
          </div>
        </div>
    ...
    <!-- WIDE MODAL SCREENS -->
    <modal class="modal-g media-query-feed-page-modal" name="hello-world">
      <div id="overlay-post">
      ...
      <form action id="post_form" @submit.prevent="createPost()" enctype="multipart/form-data">
        <div class="modal-cover">
          ...
          <div class="modal-contents">
            ...
            <div v-show="!showForum && !showStatus && !showNews" class="textfield-container post-note-post">
            ...
            <div v-show="!showForum && !showStatus && !showNews" class="cover-image-container">
              ...
              <template id v-if="currentModal !== 'video'">
                <div v-if="!imageData" class="cover-photo-1 audio-video-cover" style="width: 38%;" @click="chooseImage">
                  <span class="audio-video-cover">Upload cover photo</span>
                  <input
                    class="file-input"
                    ref="fileInput"
                    type="file"
            ...
            <div v-if="showForum" class="cover-image-container">
              <div v-if="!imageData" class="cover-photo-1" @click="chooseImage">
            ...
            <div v-if="showStatus" class="cover-image-container">
              <div v-if="!imageData" class="cover-photo-1" @click="chooseImage">
                <span>Upload Status Photo or Video</span>
                <input
                  class="file-input"
                  ref="fileInput"
                  type="file"
            ...
            <div v-if="showNews" class="cover-image-container">
              <div v-if="!imageData" class="cover-photo-1" @click="chooseImage">
            ...
            <div class="bottom-modal">
              <div class="start-icons">
                ...
                <div class="forum">
                  <img class="fast" src="../assets/post-content-icon.png" />
                  <div class="forum-news-dropdown">
                    <a @click="showModal('forum')">Forum</a>
                    <a @click="showModal('news')">News</a>
                  </div>
                </div>
                <div @click="showModal('status')">
                  <img class="fast" src="../assets/status-icon.png" />
                </div>
              </div>
    ...
    <div class="make-a-post-1">
      <div class="container-left">
        <div class="make-text">Make a Post</div>
      </div>
      <div class="container-mid">
        <div class="make-a-post">
          <div class="start-text hide-on-med-and-down">Start a post - music, photo, content</div>
          <div class="start-icons">
            <div data-target="modal1" class="modal-trigger" @click="showModal('video')">
              <img class="fast" src="../assets/music-video-icon.png" alt />
            </div>
            <div data-target="modal1" class="modal-trigger" @click="showModal('audio')">
              <img class="fast" src="../assets/music-audio-icon.png" alt />
            </div>
            <div data-target="modal1" class="modal-trigger" @click="showModal('camera')">
              <img class="fast" src="../assets/camera-icon.png" />
            </div>
            <div data-target="modal1" class="modal-trigger" @click="showModal('forum')">
              <img class="fast" src="../assets/post-content-icon.png" />
            </div>
            <div data-target="modal1" class="modal-trigger" @click="showModal('status')">
              <img class="fast" src="../assets/status-icon.png" />
            </div>
          </div>
        </div>
    ...
    <div :class="[feedOverlay ? 'sticky-feed' : '']" class="hide-on-med-and-down">
      <div class="feed-options">
        <div class="option-cover">
          <div class="option option-latest" @click.prevent="fetchPosts()">
            Latest
            <div :class="lineLatestObject"></div>
          </div>
        ...
        <div class="option-cover">
          <div class="option option-artist" @click.prevent="clicked_option('artiste')">
            Artistes
            <div :class="lineArtisteObject"></div>
          </div>
        </div>
        <div class="option-cover">
          <div class="option" @click.prevent="fetchForums()">
            Forum
            <div :class="lineForumObject"></div>
          </div>
          <div class="dropdown-content-1">
            <a href="javascript:void(0)" @click.prevent="fetchForumsViaCategory('Collaboration')">Collaboration</a>
            <a href="javascript:void(0)" @click.prevent="fetchForumsViaCategory('Music Tours')">Tours</a>
            <a href="javascript:void(0)" @click.prevent="fetchForumsViaCategory('R and B specials')">R and B</a>
            <a href="javascript:void(0)" @click.prevent="fetchForumsViaCategory('Classic meetups')">Classic</a>
            <a href="javascript:void(0)" @click.prevent="fetchForumsViaCategory('Highlife talks')">Highlife</a>
            <a href="javascript:void(0)" @click.prevent="fetchForumsViaCategory('Gospel')">Gospel</a>
          </div>
        </div>
        <div class="option-cover">
          <div class="option" @click.prevent="fetchNews()">
            News
            <div :class="lineNewsObject"></div>
          </div>
        </div>
        <div class="option-cover">
          <div class="option" @click.prevent="fetchStatus()">
            Status
            <div :class="lineStatusObject"></div>
          </div>
        </div>
        <div class="w"></div>
        <div class="w"></div>
        <div class="option-cover">
          <div class="option flex-display" @click.prevent="clicked_option('filter')">
            <div class="inner-filter">
              Filter
              <i class="fas fa-filter"></i>
            </div>
            <div class="linebeneathnc"></div>
          </div>
        </div>
      </div>
      <div :class="[feedOverlay ? 'thin-line-feed' : ' ']" class="text-wrap"></div>
    </div>
    <div data-toggle="modal" data-target="#menu-lists" class="hide-on-large-only sticky-feed-list" style="cursor: pointer; color: #00da1d; float: left; padding-top: 10px; padding-left: 5px;">
      <i class="fas fa-plus"></i> Show Menu
    </div>
    <br> <br>
    <div v-if="clicked_option_computed === 'latest'" class="feed-grid">
    ...
    <div v-if="clicked_option_computed === 'news'" class="feed-grid">
      <div class="a-feed" v-for="newNew in news" :key="newNew._id">
        <div class="feed-body">
          <div class="feed-grid-inner" @click="showNewsModal(newNew)">
            <template v-if="newNew.user.avatar == ''">
              <img class="feed-avatar" src="../assets/u7.jpg" />
            </template>
            <template v-else>
              <img class="feed-avatar" :src="newNew.user.avatar" />
            </template>
            <div class="feed-name">
              {{newNew.user.firstName}} {{newNew.user.lastName}}
              <template
                v-if="newNew.user.stageName == UserState.stageName"
              >
                <span style="color: maroon; margin: 2px 10px;" @click="deletePost(newNew._id, 'news')" class>
                  <i class="fa fa-trash"></i>
                </span>
              </template>
            </div>
            <div class="feed-time">{{newNew.timestamp | diffForHumans}}</div>
          </div>
          <h5 class="feed-text feed-text-header">{{ newNew.title }}</h5>
          <div class="feed-text" v-html="newNew.content"></div>
          <div class="feed-image">
            <img
              :src="newNew.banner"
              :alt="newNew.title"
          ...
          <div class="feed-activity">
            <div @click.prevent="likePost(newNew, newNew.liked)">
              <i @click.prevent="likePost(newNew, newNew.liked)" class="fas like-icon fa-thumbs-up icons"></i>
            </div>
            <div class="padding-act-s"></div>
            {{newNew.likes}}
            <div class="padding-act"></div>
            <div @click="redirectToCommentPage(newNew._id)">
              <i @click="redirectToCommentPage(newNew._id)" class="fas comment-icon fa-comment-alt icons"></i>
            </div>
            <div class="padding-act-s"></div>
            {{newNew.comments}}
            <div class="padding-act"></div>
            <div @click="markAsFavourite(newNew, newNew.favourite)">
              <i @click="markAsFavourite(newNew, newNew.favourite)" class="fas favourite-icon fa-heart icons"></i>
            </div>
            <div class="padding-act-s"></div>
            {{newNew.favourites}}
            <div class="padding-act-s"></div>
    ...
    <div v-if="clicked_option_computed === 'forum'" class="feed-grid">
      <div class="a-feed" v-for="(forum) in allForums" :key="forum._id">
        <template v-if="typeof forum.amount == 'undefined'">
          <div class="feed-body">
            <div class="feed-grid-inner">
              <template v-if="forum.user.avatar == ''">
                <img class="feed-avatar" src="../assets/u7.jpg" />
              </template>
              <template v-else>
                <img class="feed-avatar" :src="forum.user.avatar" />
              </template>
              <div class="feed-name">
                {{forum.user.firstName}} {{forum.user.lastName}}
                <template
                  v-if="forum.user.stageName == UserState.stageName"
                >
                  <span style="color: maroon; margin: 2px 10px;" @click="deletePost(forum._id, 'forum')" class>
                    <i class="fa fa-trash"></i>
                  </span>
                </template>
              </div>
              <div class="feed-time">{{forum.timestamp | diffForHumans}}</div>
            </div>
            <h5 class="feed-text feed-text-header">{{ forum.title }}</h5>
            <div class="feed-text" v-html="forum.content"></div>
            <div class="feed-image" v-if="forum.postType == 'video'">
              <video preload="none" class="image-feed" v-bind:poster="forum.banner" width="3000" height="240" controls>
                <source v-bind:src="forum.video" />
              </video>
            </div>
            <div class="feed-image">
              <img
            ...
            <div class="feed-activity">
              <template v-if="forum.postType == 'video'">
                <i class="fas play-icon fa-eye icons"></i>
                <div class="padding-act-s"></div>
                {{forum.views}}
                <div class="padding-act"></div>
              </template>
              <div @click.prevent="likePost(forum, forum.liked)">
                <i @click.prevent="likePost(forum, forum.liked)" class="fas like-icon fa-thumbs-up icons"></i>
              </div>
              <div class="padding-act-s"></div>
              {{forum.likes}}
              <div class="padding-act"></div>
              <div @click="redirectToCommentPage(forum._id)">
                <i @click="redirectToCommentPage(forum._id)" class="fas comment-icon fa-comment-alt icons"></i>
              </div>
              <div class="padding-act-s"></div>
              {{forum.comments}}
              <div class="padding-act"></div>
              <div @click="markAsFavourite(forum, forum.favourite)">
                <i @click="markAsFavourite(forum, forum.favourite)" class="fas favourite-icon fa-heart icons"></i>
              </div>
              <div class="padding-act-s"></div>
              {{forum.favourites}}
              <div class="padding-act-s"></div>
        ...
        <template v-if="typeof forum.amount != 'undefined'">
          <div class="feed-body">
            <div class="feed-grid-inner">
              <div class="feed-name">
                Sponsored Advert <b>{{ forum.businessName }}</b>
              </div>
            </div>
            <h5 class="feed-text feed-text-header">{{ forum.title }}</h5>
            <div class="feed-text" v-html="forum.description"></div>
            <div class="feed-image">
              <img
                :src="forum.banner"
                :alt="forum.title"
    ...
    <div v-if="clicked_option_computed === 'status'" class="feed-grid">
      <div class="a-feed" v-for="(status) in allStatus" :key="status._id">
        <div class="feed-body">
          <div class="feed-grid-inner">
            <template v-if="status.user.avatar == ''">
              <img class="feed-avatar" src="../assets/u7.jpg" />
            </template>
            <template v-else>
              <img class="feed-avatar" :src="status.user.avatar" />
            </template>
            <div class="feed-name">
              {{status.user.firstName}} {{status.user.lastName}}
              <template
                v-if="status.user.stageName == UserState.stageName"
              >
                <span style="color: maroon; margin: 2px 10px;" @click="deletePost(status._id, 'status')" class>
                  <i class="fa fa-trash"></i>
                </span>
              </template>
            </div>
            <div class="feed-time">{{status.timestamp | diffForHumans}}</div>
          </div>
          <h5 class="feed-text feed-text-header">{{ status.title }}</h5>
          <div class="feed-text" v-html="status.content"></div>
          <div class="feed-image" v-if="status.video != ''">
            <video preload="none" class="image-feed" v-bind:poster="status.banner" width="3000" height="240" controls>
              <source v-bind:src="status.video" />
            </video>
          </div>
          <div v-if="status.video == ''" class="feed-image">
            <img
              :src="status.banner"
              :alt="status.title"
              style="max-height: auto !important; width: 100%;"
          ...
          <div class="feed-activity">
            <template v-if="status.postType == 'video'">
              <i class="fas play-icon fa-eye icons"></i>
              <div class="padding-act-s"></div>
              {{status.views}}
              <div class="padding-act"></div>
            </template>
            <div @click.prevent="likePost(status, status.liked)">
              <i @click.prevent="likePost(status, status.liked)" class="fas like-icon fa-thumbs-up icons"></i>
            </div>
            <div class="padding-act-s"></div>
            {{status.likes}}
            <div class="padding-act"></div>
            <div @click="redirectToCommentPage(status._id)">
              <i @click="redirectToCommentPage(status._id)" class="fas comment-icon fa-comment-alt icons"></i>
            </div>
            <div class="padding-act-s"></div>
            {{status.comments}}
            <div class="padding-act"></div>
            <div @click="markAsFavourite(status, status.favourite)">
              <i @click="markAsFavourite(status, status.favourite)" class="fas favourite-icon fa-heart icons"></i>
            </div>
            <div class="padding-act-s"></div>
            {{status.favourites}}
            <div class="padding-act-s"></div>
    ...
    <Artistes v-else-if="clicked_option_computed === 'artiste'"></Artistes>
    <!-- SHARES MODAL -->
    ...
    <!-- NEWS MODAL -->
    <modal name="news-modal" class="news-modal">
      <div class="news-feed">
        <v-container class="lighten-5">
          <v-row no-gutters>
              <v-col
                ...
                xl="8"
                lg="8"
                class="feed-list-modal"
              >
              <div class="news-feed" v-for="modalNew in modalNews" :key="modalNew._id">
                <div class="feed-listing a-feed">
                  <div class="feed-body">
                    <div class="feed-grid-inner">
                      <template v-if="modalNew.user.avatar == ''">
                        <img class="feed-avatar" src="../assets/u7.jpg" />
                      </template>
                      <template v-else>
                        <img class="feed-avatar" :src="modalNew.user.avatar" />
                      </template>
                      <div class="feed-name">
                        {{modalNew.user.firstName}} {{modalNew.user.lastName}}
                        <template
                          v-if="modalNew.user.stageName == UserState.stageName"
                        >
                          <span style="color: maroon; margin: 2px 10px;" @click="deletePost(modalNew._id, 'news')" class>
                            <i class="fa fa-trash"></i>
                          </span>
                        </template>
                      </div>
                      <div class="feed-time">{{modalNew.timestamp | diffForHumans}}</div>
                    </div>
                    <h5 class="feed-text feed-text-header">{{ modalNew.title }}</h5>
                    <div class="feed-text" v-html="modalNew.content"></div>
                    <div class="feed-image">
                      <img
                        :src="modalNew.banner"
                        :alt="modalNew.title"
                    ...
                    <div class="feed-activity">
                      <div @click.prevent="likePost(modalNew, modalNew.liked)">
                        <i @click.prevent="likePost(modalNew, modalNew.liked)" class="fas like-icon fa-thumbs-up icons"></i>
                      </div>
                      <div class="padding-act-s"></div>
                      {{modalNew.likes}}
                      <div class="padding-act"></div>
                      <div @click="redirectToCommentPage(modalNew._id)">
                        <i @click="redirectToCommentPage(modalNew._id)" class="fas comment-icon fa-comment-alt icons"></i>
                      </div>
                      <div class="padding-act-s"></div>
                      {{modalNew.comments}}
                      <div class="padding-act"></div>
                      <div @click="markAsFavourite(modalNew, modalNew.favourite)">
                        <i @click="markAsFavourite(modalNew, modalNew.favourite)" class="fas favourite-icon fa-heart icons"></i>
                      </div>
                      <div class="padding-act-s"></div>
                      {{modalNew.favourites}}
                      <div class="padding-act"></div>
            ...
            <v-col cols="12" sm="12" md="6" lg="4" xl="4" class="related-news">
              <div class="title-heading">
                <h4>Popular News!</h4>
                <!-- News with higher ratings -->
              </div>
              <div class="relations">
                <ul v-if="popularNews.length > 0">
                  <li v-for="(popular) in popularNews" :key="popular._id">
                    <template v-if="popular.user.avatar == ''">
                      <img class="feed-avatar" src="../assets/u7.jpg" />
                    </template>
                    <template v-if="popular.user.avatar != ''">
                      <img class="feed-avatar" :src="popular.user.avatar" />
                    </template>
                    <span>{{ popular.title | truncatePopularNews }}</span> <br>
                    <small>
                      <ul>
                        <li><i class="fas fa-thumbs-up" style="color: green;"></i> {{ popular.likes }}</li>
                        <li><i class="fas fa-heart" style="color: red;"></i> {{ popular.favourites }}</li>
                        <li><i class="fas fa-comment-alt" style="color: blue;"></i> {{ popular.comments }} </li>
                      </ul>
                    </small>
                  </li>
                </ul>
    ...
    <modal name="create-playlist" style="margin-top: -5%;">
      <h5 class="black-text" style="padding-left: 10px;">Add <b>{{ playlist.title }}</b> To A Playlist.</h5>
      <hr>
          ...
          <div class="feed-body">
            <div class="feed-grid-inner" style="margin-bottom: -142px;">
              <div class="feed-name" style="margin-left: -10%;">
                ...
                <div class="left-btn" style="position: relative; left: 118%; top: -28.7%;" @click="addToPlaylist(play)">Add To Playlist</div>
                <router-link :to="{ name: 'PlayLists', params: { playlistId: play._id } }" class="left-btn" style="position: relative; left: 204%; top: -64.7%;">View Playlist</router-link>
              </div>
            </div>
          </div>
        </li>
    ...
    <!-- List Modal List -->
    <div class="modal modal-mobile-menu" style="background: transparent !important;" id="menu-lists" tabindex="-1" role="dialog">
      <div class="modal-dialog" role="document">
        <div class="modal-content">
          <div class="modal-header">
            <h5 class="modal-title" style="float: left;">Post Menu</h5>
            ...
            <div class="container-fluid">
              <div class="row">
                <div class="col-xs-12 col-sm-12">
                  <ul class="modal-menu" style="list-style: none; display: flex;">
                    <li><a href="#" @click.prevent="fetchPosts()">Latest</a></li>
                    <li><a href="#" @click.prevent="clicked_option('artiste')">Artistes</a></li>
                    <li><a href="#" @click.prevent="fetchForums()">Forum</a></li>
                    <li><a href="#" @click.prevent="fetchNews('news')">News</a></li>
                    <li><a href="#" @click.prevent="fetchStatus()">Status</a></li>
                  </ul>
                </div>
              </div>
            </div>
          ...
          <div class="modal-footer">
            <button type="button" class="btn btn-danger btn-sm z-depth-0" data-dismiss="modal"><i class="fas fa-times"></i> Close Menu</button>
          </div>
        </div>
      </div>
    </div>
...
export default {
  ...
  methods: {
    openCreateModal(media) {
      this.$modal.show('create-playlist');
      this.playlist = media
    },
    async addToPlaylist(playlistRecord) {
      let mediaIndex = playlistRecord.media.indexOf(this.playlist._id);
      if (mediaIndex == -1) {
        await axios.get(this.$baseUrl + '/add/media/playlist/' + this.playlist._id + '/' + playlistRecord._id, {
          headers: {
          ...
          if (doc.status == 200) {
            playlistRecord.media.unshift(this.playlist._id);
            this.$toast.success(this.playlist.title + ' has been added Successfully to your playlist(' + playlistRecord.name + ')', this.playlist.postType + ' added', this.notificationSystem.options.success);
          }
        }).catch((e) => {
          this.$toast.error('This ' + this.playlist.postType + ' could not be added to the playlist.', 'Server Error', this.notificationSystem.options.error);
        });
        return;
      }

      this.$toast.info('This ' + this.playlist.postType + ' already exists in this playlist.', 'Duplicate Entry', this.notificationSystem.options.info);
      return;
    },
        ...
        if (response.status !== 400 && response.status !== 500) {
          ...
          if (payload.type == 'like' && response.data.data[0].type.likes) {
            ...
            if (post.postType == 'forum') {
              this.allForums = this.allForums.map((el) => {
                if (el._id == post._id) {
                  el.liked = true;
                  el.likes = parseInt(el.likes) + 1;
                }
            ...
            if (post.postType == 'status') {
              this.allStatus = this.allStatus.map((el) => {
                if (el._id == post._id) {
                  el.liked = true;
                  el.likes = parseInt(el.likes) + 1;
                }
        ...
        .then(response => {
          if (response.status == 200) {
            ...
            if (postType == 'news') {
              ...
              eventBus.$emit("post-action", { type: "decrement" });
              this.$toast.info('The selected news has been deleted permanently.', 'News Deleted', this.notificationSystem.info);
            }

            if (postType == 'status') {
              this.allStatus = this.allStatus.filter(el => {
                if (el._id != postId) return true;
              });

              // eventBus.$emit("post-action", { type: "decrement" });
              this.$toast.info('The selected Status has been removed Successfully.', 'Status Removed', this.notificationSystem.info);
            }

            if (postType == 'forum') {
              this.allForums = this.allForums.filter(el => {
                if (el._id != postId) return true;
              });

              eventBus.$emit("post-action", { type: "decrement" });
              this.$toast.info('The selected Thread has been Deleted Successfully.', 'Thread Deleted', this.notificationSystem.info);
            }
          }
        })
        .catch(e => {
  ...
  data() {
    return {
      ...
      selected_contest: "Select Contest",
      displayed: "",
...
@media (max-width:320px) {
  .media-query-feed-page-modal .modal-cover {
    width: 85%;
    right: 11px;
    top: 135px;
  }
  .margin-query-feed-page-modal .audio-video-cover {
    width: 5% !important;
  }
  .media-query-feed-page-modal .cover-image-container input {
    margin-left: 25%;
  }
  .media-query-feed-page-modal .start-icons {
    margin-left: -5%;
  }
  .media-query-feed-page-modal .start-icons img {
    width: 93%;
    padding: 0px 4px;
  }
  .media-query-feed-page-modal .originality {
    margin-left: 3%;
  }
  .media-query-feed-page-modal #publish_button .publishbutton {
    margin-right: -15px;
  }
  .media-query-feed-page-modal .list-media-list {
    width: 95%;
  }
}
